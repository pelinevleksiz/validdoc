...
- Replaced Otsu-threshold-based blank/ink detection (unreliable on noisy scans) with pixel-standard-deviation-based detection, applied uniformly to text and ink segments. Added a narrow, field-type-specific character whitelist and single-line page segmentation mode for phone and email segments only (broader application of either was tested and reverted as harmful). Corrected a known Turkish-language Tesseract limitation where "@" is frequently misread as "©" in email fields. OCR confidence is now surfaced in the document detail UI for admins. Final measured segment-level accuracy: clean 100.0%, medium 90.2%, bad 88.0% (from a pre-Item-21 baseline of 81.5%/38.0%/26.1%).
- Rewrote SRS to a fully IEEE 830-1998-compliant structure: stakeholder analysis, user-story-based functional requirements with Given/When/Then acceptance criteria, Essential/Conditional/Optional ranking, tabular use case descriptions for core workflows, and supporting Mermaid diagrams (system context, use case, sequence, state, activity, class).
